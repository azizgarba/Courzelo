...
export class IncentivesComponent {
  ...
  ngOnInit(): void {
        ...
        //
    this.voteService.getIncentivesByTeacher(this.idUser).subscribe((data:Incentives[])=>{
      this.listIncentives=data;
    });
    this.voteService.getBadgesByTeacher(this.idUser).subscribe((data:BadgeForumTeacher[])=>{
